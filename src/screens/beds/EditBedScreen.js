...
import { updateBed, clearBedState } from '../../redux/slices/bedSlice';
...
const EditBedScreen = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await dispatch(updateBed({ bedId: bed._id, updateData: formData })).unwrap();
    } catch (error) {
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Card style={styles.card}>
          <Card.Content>
            <Text style={styles.title}>Chỉnh sửa Giường</Text>
            <Text style={styles.subtitle}>Giường: {bed?.bed_number}</Text>
            ...
            >
              {isLoading ? 'Đang cập nhật...' : 'Cập nhật giường'}
            </Button>
          </Card.Content>
        </Card>
      </ScrollView>
