...
const ChangePasswordScreen = ({ navigation }) => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const response = await authService.changePassword(currentPassword.trim(), newPassword.trim());
      ...
      if (response.success) {
        ...
        throw new Error(response.error || 'Không thể thay đổi mật khẩu');
      }
    } catch (error) {
      console.error('Change password error:', error);
      Alert.alert(
        'Lỗi',
        error.response?.data?.message || error.message || 'Không thể thay đổi mật khẩu. Vui lòng thử lại.'
      );
    } finally {
      setLoading(false);
    }
...
const styles = StyleSheet.create({
  ...
  scrollContentContainer: {
    flexGrow: 1,
    paddingBottom: 40, // Thêm padding bottom để tránh bị che bởi keyboard
  },
  keyboardAvoidingView: {
    flex: 1,
  },
